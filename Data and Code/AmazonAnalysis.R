...
# Create an EDA dataset and correct Vroom's mistake by making numeric data into factors
...
# Create logistic regression model using GLM function
...
# Predict with classification cutoff = .83 since each prediction needs to be binary
...
# Predict without a classification cutoff--just the raw probabilities instead of binaries
...
plogr_mod <- logistic_reg(mixture = tune(), # tune mixture and penalty
                          penalty = tune()) %>%
  set_engine("glmnet")
...
ctree_mod <- rand_forest(mtry = tune(), # Tune mtry and min_n; trees stay at 500
                         min_n = tune(),
                         trees = 500) %>%
...
# Create Naive Bayes model
nbpcr_mod <- naive_Bayes(Laplace = tune(), # Tune laplace and smoothness
                      smoothness = tune()) %>%
  set_mode("classification") %>%
  set_engine("naivebayes") # Use naivebayes function
...
# Create K Nearest Neighbors model
knnpcr_mod <- nearest_neighbor(neighbors = tune()) %>% # Tune number of neighbors
  set_mode("classification") %>%
  set_engine("kknn") # Use kknn function
...
# Create classification forest model
ctree_mod <- rand_forest(mtry = tune(), # Tune mtry and min_n; Always use 750 trees
                         min_n = tune(),
                         trees = 750) %>%
  set_engine("ranger") %>% # Use ranger function
  set_mode("classification")
...
# Create classification forest model
ctreepcr_mod <- rand_forest(mtry = tune(), # Tune mtry and min_n; Always use 1000 trees
                         min_n = tune(),
                         trees = 1000) %>%
  set_engine("ranger") %>%
  set_mode("classification")
...
# Create penalized logistic regression model
plogrpcr_mod <- logistic_reg(mixture = tune(), # Tune mixture and penalty
                          penalty = tune()) %>%
  set_engine("glmnet")
...
plogrpcr_mod <- logistic_reg(mixture = tune(), # Tune mixture and penalty
                          penalty = tune()) %>%
  set_engine("glmnet") # Use glmnet function
...
# Create classification forest model
ctree_mod <- rand_forest(mtry = tune(), # Tune mtry and min_n and keep trees at 750
                         min_n = tune(),
                         trees = 750) %>%
  set_engine("ranger") %>% # Use ranger function
  set_mode("classification")
...
# Create SVMS model
svms_radial_mod <- svm_rbf(rbf_sigma = tune(), # Tune rbf_sigma and cost
                           cost = tune()) %>%
  set_mode("classification") %>%
  set_engine("kernlab")
...
# Create classification forest model
ctree_mod <- rand_forest(mtry = tune(), # Tune mtry and min_n; Keep trees at 750
                         min_n = tune(),
                         trees = 750) %>%
  set_engine("ranger") %>%
  set_mode("classification")
...
# Create SVMS model
svms_linear_mod <- svm_linear(cost = tune()) %>% # Tune cost
  set_mode("classification") %>%
  set_engine("kernlab")
...
#################################################################
#################################################################
# MEGA FOREST -- Just a really detailed random forest   #########
#################################################################
#################################################################
...
rf_mod <- rand_forest(mtry = tune(), # Tune mtry and min_n but keep trees at 1000
                         min_n = tune(),
                         trees = 1000) %>%
  set_engine("ranger") %>%
  set_mode("classification")
